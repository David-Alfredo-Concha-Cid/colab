...
class Command(BaseCommand):
    ...
    def seeds_student(self, mode):
        if mode == "load" and Student.objects.all().count() <= 0:
            ...
            s.save()
            perm = Permission.objects.get(name = "Can add subscription") #capatura el permiso
            s.user_permissions.add(perm) # le da el permiso
...
# crearemos una clase que permitira crear un comando personalizado 